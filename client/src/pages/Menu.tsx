...
const Menu: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user) {
      fetch(`${BASE}/favourites/ids`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      })
        .then((r) => r.json())
        .then((d) => setFavIds(Array.isArray(d) ? d : []))
        .catch(() => {});
    }
  }, [user]);
  ...
  const toggleFav = async (productId: number) => {
    ...
    if (favIds.includes(productId)) {
      await fetch(`${BASE}/favourites/${productId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      setFavIds((prev) => prev.filter((id) => id !== productId));
    } else {
      await fetch(`${BASE}/favourites`, {
        method: "POST",
        headers: {
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          ...
          <p className={styles.heroSub}>
            Hand-crafted and baked fresh each morning. Pick your favourites.
          </p>
        </div>
      </section>
